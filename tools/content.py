...
EN = {
  ...
  "js": {
    ...
    "log": ["<span class='dim'>read</span>   receipt-0412.pdf   <span class='ok'>on device</span>",
            ...
            "<span class='dim'>ask</span>    \"receipt for tax?\" <span class='ok'>Loupe · local</span>",
...
AR = {
  ...
  "js": {
    ...
    "log": ["<span class='dim'>قراءة</span>  receipt-0412.pdf   <span class='ok'>على الجهاز</span>",
            ...
            "<span class='dim'>سؤال</span>   «إيصال للضرائب؟»   <span class='ok'>Loupe · محلي</span>",
...
HOME = {
 "en": {
  ...
  "priv_p": "The Loupe Decision Model runs on your iPhone or Mac. Your photos, files, calendar and contacts are read there and are not sent to us or anyone else. No account. No analytics, advertising or crash-reporting SDKs.",
  ...
  "run_p2": "<b>Riverflight</b> is a river shooter where Loupe flies. It decides many times a second with its raw probability bars on screen, against a dumb autopilot on the same river.",
  "run_stats": [("60 fps", "steady, Loupe flying"), ("10–14/s", "decisions"), ("~63 ms", "p50 latency"), ("~82 ms", "p95 latency")],
  "run_src": "Measured 25 Sep 2026.",

  "dev_eyebrow": "The apps",
  "dev_h": "Real screens, dark neon",
  "dev_p": "Screenshots from Loupe for iPhone.",
  "shots": [("now", "Now", "Now: the top finding, what needs you, and “On this phone. Nothing leaves it.”"),
            ...
            ("game", "Riverflight", "Riverflight results: decisions per second, p50/p95 latency and the decision model against the baseline."),
 ...
 "ar": {
  ...
  "priv_p": "نموذج Loupe للقرار يعمل على iPhone أو Mac. تُقرأ صورك وملفاتك وتقويمك وجهات اتصالك هناك ولا تُرسَل إلينا ولا إلى أي أحد. لا حساب. لا أدوات تحليلات أو إعلانات أو تقارير أعطال.",
  ...
  "run_p2": "<b>Riverflight</b> لعبة إطلاق نار على نهر يقودها Loupe. يقرّر مرات عديدة في الثانية وأشرطة احتمالاته الخام على الشاشة، في مواجهة طيّار آلي بسيط على النهر نفسه.",
  "run_stats": [("60 fps", "ثابتة، Loupe يقود"), ("10–14/ث", "قرارًا"), ("~63 ms", "زمن p50"), ("~82 ms", "زمن p95")],
  "run_src": "قيست في 25 سبتمبر 2026.",

  "dev_eyebrow": "التطبيقات",
  "dev_h": "شاشات حقيقية، نيون داكن",
  "dev_p": "لقطات من Loupe لأجهزة iPhone.",
  "shots": [("now", "الآن", "الآن: أهم نتيجة، وما يحتاجك، و«على هذا الهاتف. لا شيء يغادره.»"),
            ...
            ("game", "Riverflight", "نتائج Riverflight: القرارات في الثانية وزمن p50/p95 ونموذج القرار مقابل الأساس."),
  ...
  "mac_ph": "Loupe Station لأجهزة Mac",
...
DIFFS = {
 "en": [
  ("words", "Judgments in plain words, run on the device", "Write a question the way you'd ask a friend. The Loupe Decision Model (~320M parameters) turns it into a yes/no, pick-one or score decision and runs it on your phone or Mac. It doesn't generate text; it decides.", "~10 decisions/s, on-device"),
  ...
  ("game", "Riverflight — watch the model decide live", "A river shooter where Loupe flies, with its raw probability bars on screen, against a dumb autopilot on the same seed.", "60 fps · p50 ~63 ms"),
 ...
 "ar": [
  ("words", "أحكام بكلمات بسيطة، تعمل على الجهاز", "اكتب سؤالًا كما تسأل صديقًا. يحوّله نموذج Loupe للقرار (~320 مليون معامل) إلى قرار نعم/لا أو اختيار أو درجة، ويشغّله على هاتفك أو Mac. لا يولّد نصًا؛ بل يقرّر.", "~10 قرارات/ث على الجهاز"),
  ...
  ("game", "Riverflight — شاهد النموذج يقرّر مباشرةً", "لعبة نهر يقودها Loupe وأشرطة احتمالاته الخام على الشاشة، في مواجهة طيّار آلي بسيط على البذرة نفسها.", "60 fps · p50 ~63 ms"),
...
FEATURES = {
 "en": {
  ...
  "ios_h": "Loupe for iPhone", "ios_sub": "iOS 17+ · English and Arabic",
  "mac_h": "Loupe Station for Mac", "mac_sub": "macOS 14+ · Apple Silicon · powered by the Loupe Decision Model",
  "ios": [
   ...
   ("Web templates (Online, opt-in)", "Currency, weather and UK trains, with ready questions in English and Arabic or your own; the decision model ranks the fetched rows on the phone, with the rule baseline beside it."),
   ...
   ("Riverflight", "Fly it yourself or watch Loupe fly, with raw probability bars, a speed panel and a results card against the baseline."),
   ("Export my data", "Your decisions, corrections and settings as a zip — portable files you own."),
   ("The Loupe Decision Model", "A consented one-time download (~418 MB, SHA-256 checked). 322M parameters, Apache-2.0 weights, 100+ languages."),
  ],
  "mac": [
   ("Playground", "Ask the decision model typed questions (choice, score, yes/no) about any text or JSON and see every probability. About 40 ms a decision on Apple silicon."),
   ...
   ("Decision ledger", "Every decision with its full distribution, so accuracy and thresholds can be measured."),
   ("Measure: the decision model against simple rules", "Each judgment compared with its keyword baseline on your data."),
 ...
 "ar": {
  ...
  "ios_h": "Loupe لأجهزة iPhone", "ios_sub": "iOS 17 أو أحدث · العربية والإنجليزية",
  "mac_h": "Loupe Station لأجهزة Mac", "mac_sub": "macOS 14 أو أحدث · Apple Silicon · يعمل بنموذج Loupe للقرار",
  "ios": [
   ...
   ("قوالب الويب (عبر الإنترنت، اختيارية)", "العملات والطقس وقطارات بريطانيا، بأسئلة جاهزة بالعربية والإنجليزية أو سؤالك؛ يرتّب نموذج القرار الصفوف المجلوبة على الهاتف مع قاعدة الأساس بجانبها."),
   ...
   ("Riverflight", "قُدها بنفسك أو شاهد Loupe يقودها، مع أشرطة الاحتمالات الخام ولوحة السرعة وبطاقة نتائج مقابل الأساس."),
   ("صدّر بياناتي", "قراراتك وتصحيحاتك وإعداداتك في ملف zip — ملفات قابلة للنقل تملكها."),
   ("نموذج Loupe للقرار", "تنزيل لمرة واحدة بموافقتك (~418 ميغابايت، مع تحقق SHA-256). 322 مليون معامل، أوزان Apache-2.0، أكثر من 100 لغة."),
  ],
  "mac": [
   ("ساحة التجربة", "اسأل نموذج القرار أسئلة محدّدة النوع (اختيار، درجة، نعم/لا) عن أي نص أو JSON وشاهد كل احتمال. نحو 40 ms للقرار على Apple silicon."),
   ...
   ("القياس: نموذج القرار مقابل القواعد البسيطة", "كل حُكم يُقارن بأساس الكلمات المفتاحية على بياناتك."),
